...
function init (client, $) {
  ...
  function setDateAndTime (time) {
    ...
    // eventTime.val(time.hours() + ":" + time.minutes());  // Broken: Often populates time field with '--:--:--'
    eventTime.val(time.format('HH:mm')); // Fixed?
    // eventDate.val(time.toISOString().split('T')[0]);  // Broken: Returns date string in UTC
    eventDate.val(time.format('YYYY-MM-DD')); // Fixed: Returns date string in current time zone
  }
